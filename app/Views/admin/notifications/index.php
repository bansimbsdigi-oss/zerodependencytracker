...
<div class="notif-tabs">
  <a href="<?= APP_URL ?>/admin/notifications?tab=client" class="notif-tab <?= $activeTab === 'client' ? 'active' : '' ?>">
    Client Notifications
    <?php if ($clientUnread): ?><span class="notif-badge"><?= $clientUnread ?></span><?php endif; ?>
  </a>
  <a href="<?= APP_URL ?>/admin/notifications?tab=team" class="notif-tab team <?= $activeTab === 'team' ? 'active' : '' ?>">
    Team Notifications
...
<div class="notif-section">
  ...
  <?php foreach ($clientItems as $n):
    ...
    $isWarn     = $isFeedback;
    $targetUrl  = $n->related_user_id ? APP_URL . '/admin/clients/view/' . (int)$n->related_user_id . ($n->related_feedback_id ? '#feedback-' . (int)$n->related_feedback_id : '') : '#';
  ?>
...
<div class="notif-section">
  ...
  <div class="notif-row <?= $n->is_read ? 'is-read' : 'is-unread' ?>">
    ...
    <div class="notif-actions">
      <?php if ($n->related_user_id): ?>
        <a class="admin-row-btn outline" href="<?= APP_URL . '/admin/clients/view/' . (int)$n->related_user_id ?>">View Client</a>
      <?php endif; ?>
      <?php if (!$n->is_read): ?>
        <form method="POST" action="<?= APP_URL ?>/admin/notifications">
          <input type="hidden" name="csrf_token" value="<?= e(getCsrf()) ?>">
