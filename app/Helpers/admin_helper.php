...
if (!function_exists('adminIcon')) {
    function adminIcon(string $name, int $size = 20): string {
        $s     = $size;
        $icons = [
...
if (!function_exists('adminSidebarLink')) {
    function adminSidebarLink(string $href, string $label, string $activeKey, string $active, string $icon = '', string $badge = ''): void {
...
if (!function_exists('adminPageStart')) {
...
<div class="admin-layout">
  <aside class="admin-sidebar" id="adminSidebar">
    ...
    <nav class="admin-nav">
      ...
      <?php adminSidebarLink('/admin/dashboard', 'Dashboard', 'dashboard', $active, adminIcon('dashboard')); ?>

      <?php if (hasPermission('view_clients')): ?>
        <div class="admin-nav-section">Clients</div>
        <?php adminSidebarLink('/admin/clients', 'All Clients', 'clients', $active, adminIcon('clients')); ?>
      <?php endif; ?>

      <div class="admin-nav-section">Content</div>
      <?php if (hasPermission('manage_questions')): ?>
        <?php adminSidebarLink('/admin/questions', 'Questions', 'questions', $active, adminIcon('questions')); ?>
        <?php adminSidebarLink('/admin/sections', 'Sections', 'sections', $active, adminIcon('sections')); ?>
      <?php endif; ?>
      <?php if (hasPermission('manage_areas')): ?>
        <?php adminSidebarLink('/admin/areas', 'Problem Areas', 'areas', $active, adminIcon('areas')); ?>
      <?php endif; ?>
      <?php if (hasPermission('manage_mappings')): ?>
        <?php adminSidebarLink('/admin/mappings', 'Q-Area Mapping', 'mappings', $active, adminIcon('mappings')); ?>
      <?php endif; ?>

      <?php if (hasPermission('view_scores')): ?>
        <div class="admin-nav-section">Audits</div>
        <?php adminSidebarLink('/admin/audits', 'Audit Windows', 'audits', $active, adminIcon('audits')); ?>
      <?php endif; ?>

      <?php if ($role === 'admin'): ?>
        <div class="admin-nav-section">Team</div>
        <?php adminSidebarLink('/admin/team', 'Team Members', 'team', $active, adminIcon('team')); ?>
      <?php endif; ?>

      <div class="admin-nav-section">System</div>
      <?php adminSidebarLink('/admin/notifications', 'Notifications', 'notifications', $active, adminIcon('bell'), $unread ? (string)$unread : ''); ?>
    </nav>
  ...
  <main class="admin-main">
    <header class="admin-header">
      ...
      <div class="admin-header-actions">
        <a href="<?= APP_URL ?>/admin/notifications" class="admin-bell" aria-label="Notifications"><?= adminIcon('bell', 22) ?><?= $unread ? '<span>' . (int)$unread . '</span>' : '' ?></a>
        <form method="POST" action="<?= APP_URL ?>/admin/logout" style="display:inline;">
...
if (!function_exists('teamNotify')) {
    function teamNotify(string $type, string $message, ?int $relatedUserId = null, ?int $relatedAuditId = null): void {
        ...
        $pdo     = getDB();
        $pdo->prepare('INSERT INTO admin_notifications (type, category, message, related_user_id, related_audit_session_id, related_admin_id) VALUES (?, \'team\', ?, ?, ?, ?)')
            ->execute([$type, $message, $relatedUserId, $relatedAuditId, $actorId]);
    }
}

if (!function_exists('adminCanAccessClient')) {
    function adminCanAccessClient(int $clientId): bool {
        if (($_SESSION['admin_role'] ?? '') === 'admin') return true;
        $pdo  = getDB();
