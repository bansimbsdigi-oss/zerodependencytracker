<?php
// includes/audit_reminders.php

require_once __DIR__ . '/whatsapp.php';

function getAuditReminderParams($user, $window) {
...
function sendAuditReminders(PDO $pdo) {
    $windowStmt = $pdo->query("SELECT * FROM audit_windows WHERE is_open = 1");
    $openWindows = $windowStmt->fetchAll();
    ...
    $notStartedStmt = $pdo->prepare(
        "SELECT u.id, u.name, u.mobile
         FROM users u
         WHERE u.is_graduated = 0
           AND u.mobile <> ''
           AND NOT EXISTS (
             SELECT 1 FROM audit_sessions aus WHERE aus.user_id = u.id AND aus.audit_window_id = ?
           )"
    );
    ...
    $inProgressStmt = $pdo->prepare(
        "SELECT u.id, u.name, u.mobile
         ...
         WHERE aus.audit_window_id = ?
           AND aus.status = 'in_progress'
